...
public class FitAndroidAssetsToCacheUtil3 {
    ...
    public static String get_file_url() {
        String file_apk_url;
        File file_apks = BaseApp3.get().getExternalCacheDir();
        if (file_apks != null) {
            file_apk_url = file_apks.getAbsolutePath();
        } else {
            file_apk_url = BaseApp3.get().getExternalFilesDir(null).getAbsolutePath();
        }
        return file_apk_url;
    }
     ...
     * @return
     */
    public static File getAssetsHuanCunLujing(Context activity, String file_name, String file_assets_lujing) {
     ...
     * @param assets_url
     */
    public static void readAssertResource(Context activity, String strAssertFileName, String assets_url) {
        String all_assets_path = get_file_url() + FILE_SEP + assets_url;
        FileUtils.createOrExistsFile(all_assets_path + strAssertFileName);
        AssetManager assetManager = BaseApp3.get().getAssets();
        try {
...
//        File apk = new File(apkPath, "apk.apk");
    ...
    public static void takePhotoNoCompress(Activity activity, File file) {
        ...
        if (takePictureIntent.resolveActivity(activity.getPackageManager()) != null) {
            Uri fileUri = FileProvider73.getUriForFile(activity, file);
...
//        File file = new File(getExternalFilesDir(null), "app-debug.apk");
        ...
        FileProvider73.setIntentDataAndType(activity, intent, "application/vnd.android.package-archive", file, true);
